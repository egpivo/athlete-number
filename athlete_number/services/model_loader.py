...
class ModelLoader:
    """Download a PyTorch model file, without checksum verification."""
    ...
    async def _download_async(self) -> None:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(self.model_url, follow_redirects=True)
    ...
    def download_model(self) -> str:
        """Main entry: creates directory, downloads if needed, returns file path."""
        try:
            self.model_dir.mkdir(exist_ok=True)
            if self._should_download():
                self._download()
